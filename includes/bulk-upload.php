...
function arnes_s3_scan_media_library( $filters = [] ) {
	// Uporabimo WP-idiomatic get_posts() namesto direktnega SQL - varno in brez SQL injekcij
	$query_args = [
	...
	// Filter po datumu - WordPress date_query API
	...
	// Filter po MIME tipu - WP_Query post_mime_type podpira prefix "image" → "image/*"
	if ( ! empty( $filters['mime_type'] ) && 'all' !== $filters['mime_type'] ) {
		$query_args['post_mime_type'] = sanitize_text_field( $filters['mime_type'] );
	}

	// Cache ključ temelji na filtrih
	$cache_key   = 'arnes_s3_scan_' . md5( wp_json_encode( $query_args ) );
